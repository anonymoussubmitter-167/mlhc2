...
def fig1():
    ...
    outer = fig.add_gridspec(1, 2, width_ratios=[4.3, 1.6], wspace=0.60,
                             left=0.07, right=0.99, top=0.82, bottom=0.08)
    ...
    ax_h.set_xticklabels(col_order, fontsize=11, rotation=0, ha="center")
    ...
    bar_colors = [CB["vermilion"] if c == "Age" else CB["sky"] for c in col_order]
    ...
    for i, val in enumerate(means_vals):
        fc = CB["vermilion"] if col_order[i] == "Age" else "0.3"
        fw = "bold" if col_order[i] == "Age" else "regular"
        ax_bar.text(val + 0.003, i, f"{val:.3f}",
                    va="center", fontsize=13, color=fc, fontweight=fw)

    ax_bar.set_yticks(y_pos)
    ax_bar.set_yticklabels(col_order, fontsize=10)
    ax_bar.set_xlabel("Avg. AUROC Gap\nacross scores", fontsize=10)
